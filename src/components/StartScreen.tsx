...
export default function StartScreen({ onUnlock }: StartScreenProps) {
  ...
  return (
    <div className="glass-panel" style={{ maxWidth: "500px", margin: "10vh auto" }}>
      <h1 style={{ marginBottom: "0.5rem" }}>MPass Desktop</h1>

      {action.type === 'idle' ? (
        <>
          <p style={{ color: "var(--text-secondary)", marginBottom: "2rem" }}>Select a vault to open or create a new one.</p>

          {error && <div style={{ color: "var(--danger-color)", fontSize: "0.9rem", marginBottom: "1rem" }}>{error}</div>}

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            <button onClick={handleCreateClick} disabled={loading}>Create New</button>
            <button onClick={handleImportClick} className="secondary" disabled={loading}>Open Vault</button>
          </div>

          {recents.length > 0 && (
            <div style={{ marginTop: "3rem", textAlign: "left" }}>
              <h4 style={{ color: "var(--text-secondary)", marginBottom: "1rem" }}>Recent Vaults</h4>
        ...
        <form onSubmit={executeAction} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <p style={{ color: "var(--text-secondary)", marginBottom: "1rem" }}>
            {action.type === 'create' ? "Set a master password for your new vault." : "Enter master password to unlock vault."}
            <br />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginTop: "1rem" }}>
            <button type="submit" disabled={loading}>
              {action.type === 'create' ? "Create Vault" : "Unlock"}
            </button>
            <button type="button" onClick={cancelAction} className="secondary" disabled={loading}>Cancel</button>
          </div>
        </form>
      )}
    </div>
